...
function konfirmasiQRIS() {
    ...
    // KEMBALIKAN TAMPILAN KE CASH SETELAH BERHASIL
    document.getElementById("qrisSection").style.display = "none";
    document.querySelector('input[name="metode"][value="cash"]').checked = true;
    
    // FIX: Tampilkan kembali seluruh area pembayaran Cash
    document.querySelector(".bayar-section").style.display = "flex";
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // FIX EVENT LISTENER: Sembunyikan/Tampilkan seluruh elemen .bayar-section
    document.querySelectorAll('input[name="metode"]').forEach(r => {
        r.addEventListener("change", function() {
            const bayarSection = document.querySelector(".bayar-section");
            const qrisSection = document.getElementById("qrisSection");
            const uangBayarInput = document.getElementById("uangBayar");
            
            if (this.value === "qris") {
                // Sembunyikan input uang DAN tombol "Bayar" Cash
                bayarSection.style.display = "none";
                qrisSection.style.display = "flex";
                document.getElementById("kembalian").innerText = "0";
                uangBayarInput.value = "";
            } else {
                // Munculkan kembali input uang DAN tombol "Bayar" Cash
                bayarSection.style.display = "flex";
